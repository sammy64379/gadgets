...
def update_db():
    ...
    for name, urls in data.items():
        ...
        if download_image(urls, filename):
            ...
            cursor.execute("SELECT name FROM items WHERE name LIKE ? or name LIKE ?", (f"%{search_name}%", f"%{name}%"))
            row = cursor.fetchone()
            if row:
                actual_name = row[0]
                cursor.execute("UPDATE items SET image = ? WHERE name = ?", (new_path, actual_name))
                if cursor.rowcount > 0:
                    print(f"  SUCCESS: Updated DB with {new_path}")
            else:
                print(f"  WARNING: Item '{search_name}' or '{name}' not found in DB to update.")
        else:
