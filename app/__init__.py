...
def build_category_data():
    ...
    for (cat,) in distinct:
        if not cat:
            continue
        categories.append(
            {"value": cat, "label": CATEGORY_LABELS.get(cat, cat.title())}
        )
...
def build_home_context(
    items, search=False, query=None, selected_category="All", show_catalog=False
):
    categories = build_category_data()
    hero_item = (
        Item.query.filter(Item.category == "Apple").order_by(Item.id.desc()).first()
    )
    if not hero_item:
    ...
    selected_label = (
        ...
        else CATEGORY_LABELS.get(selected_category, selected_category.title())
    )
    return dict(
        items=items,
        hero_item=hero_item,
    ...
    reviews = [
        {
            ...
            "rating": 5,
            "time": "2 days ago",
            "comment": "Authentic product, sealed box and the M-Pesa checkout was seamless.",
        },
        {
            "name": "Brian O.",
            "rating": 4,
            "time": "1 week ago",
            "comment": "Delivered same day in Nairobi CBD. Helpful support team with setup.",
        },
        {
            "name": "Naomi N.",
            "rating": 5,
            "time": "3 weeks ago",
            "comment": "Best price I found locally. Highly recommend Ebenezer Electronics.",
        },
    ]
    ...
    faqs = [
        {
            ...
            "answer": "Yes, we ship countrywide within 24-48 hours via G4S or Fargo. Delivery fees depend on location.",
        },
        {
            "question": "Can I pay on delivery?",
            "answer": "For Nairobi we support pay-on-delivery via M-Pesa. Upcountry orders require a 30% deposit.",
        },
        {
            "question": "Is the warranty valid in Kenya?",
            "answer": "Absolutely. All devices come with official manufacturer warranty honoured locally.",
        },
    ]
...
def home():
    ...
    if selected_category and selected_category.lower() != "all":
        items_query = items_query.filter(
            func.lower(Item.category) == selected_category.lower()
        )
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash(
            "An email has been sent with instructions to reset your password. (Check console for the link)",
            "success",
        )
        return redirect(url_for("login"))
...
def mpesa_checkout():
    ...
    base_callback = os.environ.get(
        "MPESA_CALLBACK_URL",
        "https://semiexpositive-amiee-refractorily.ngrok-free.dev/mpesa_callback",
    )
    ...
    if res.get("ResponseCode") == "0":
        flash(
            "M-Pesa payment initiated. Check your phone to complete the payment.",
            "success",
        )
