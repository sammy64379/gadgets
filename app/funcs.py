...
def fulfill_order(session):
    ...
    pending_order = Order.query.filter_by(uid=uid, status="Pending").first()
    if pending_order:
        pending_order.status = "Paid"
        pending_order.date = datetime.datetime.now()
        ...
        order = Order(uid=uid, date=datetime.datetime.now(), status="Paid")
